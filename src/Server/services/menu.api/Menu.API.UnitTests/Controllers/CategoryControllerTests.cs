...
namespace Menu.API.UnitTests.Controllers
{
	public class CategoryControllerTests : BaseAutoMockedTest<CategoriesController>
	{
		...
		public void Get_should_return_category_dtos(List<Category> categories, List<CategoryDto> categoryDtos)
		{
			...
			GetMock<IMapper>().Setup(x => x.Map<IEnumerable<CategoryDto>>(categories)).Returns(categoryDtos);
		...
		public void Given_valid_id_Get_should_return_some_specific_Category_Dto(Fixture fixture)
		{
			...
			GetMock<IMapper>().Setup(x => x.Map<CategoryDto>(category)).Returns(categoryDto);
		...
		{
			GetMock<IMapper>().Setup(x => x.Map<Category>(categoryDto)).Returns(category);
