...
;
			...
			fileInfoFacade.Setup(x => x.GetFilePathWithWebRoot(Folders.UploadFilesPath, uniqFileName)).Returns(fullPath);
			fileInfoFacade.Setup(x => x.GetFileStream(fullPath, FileMode.Create)).Returns(Stream.Null);
            fileInfoFacade.Setup(x => x.Exists(uniqId)).Returns(true);
