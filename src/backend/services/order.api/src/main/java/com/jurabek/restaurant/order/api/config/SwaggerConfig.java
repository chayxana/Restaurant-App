...
public class SwaggerConfig {
  ...
  public Docket api() {
    ...
    return new Docket(DocumentationType.SWAGGER_2)
        .select()
        .apis(RequestHandlerSelectors.basePackage("com.jurabek.restaurant.order.api"))
        .paths(x -> !Objects.equals(x, "/error"))
        .build()
        .pathProvider(
            new RelativePathProvider(this.servletContext) {
              @Override
  ...
  private SecurityScheme securityScheme() {
    GrantType grantType =
        new ImplicitGrantBuilder()
            .loginEndpoint(new LoginEndpoint(identityUrl + "/connect/authorize"))
            .build();
